<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="流程分类(如：请假、报销等)" prop="category">
        <el-input
          v-model="queryParams.category"
          placeholder="请输入流程分类(如：请假、报销等)"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      ...
      <el-form-item label="是否启用(1启用，0停用)" prop="isActive">
        <el-input
          v-model="queryParams.isActive"
          placeholder="请输入是否启用(1启用，0停用)"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      ...
      <el-form-item label="最少审批人数" prop="minApprovers">
        <el-input
          v-model="queryParams.minApprovers"
          placeholder="请输入最少审批人数"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="processesList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="流程分类(如：请假、报销等)" align="center" prop="category" />
      <el-table-column label="流程版本" align="center" prop="version" />
      <el-table-column label="是否启用(1启用，0停用)" align="center" prop="isActive" />
      <el-table-column label="创建人ID" align="center" prop="creatorId" />
      <el-table-column label="最少审批人数" align="center" prop="minApprovers" />
      <el-table-column label="审批策略" align="center" prop="approvalStrategy" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="流程分类(如：请假、报销等)" prop="category">
          <el-input v-model="form.category" placeholder="请输入流程分类(如：请假、报销等)" />
        </el-form-item>
        <el-form-item label="流程版本" prop="version">
          <el-input v-model="form.version" placeholder="请输入流程版本" />
        </el-form-item>
        <el-form-item label="是否启用(1启用，0停用)" prop="isActive">
          <el-input v-model="form.isActive" placeholder="请输入是否启用(1启用，0停用)" />
        </el-form-item>
        <el-form-item label="创建人ID" prop="creatorId">
          <el-input v-model="form.creatorId" placeholder="请输入创建人ID" />
        </el-form-item>
        <el-form-item label="最少审批人数" prop="minApprovers">
          <el-input v-model="form.minApprovers" placeholder="请输入最少审批人数" />
        </el-form-item>
        <el-form-item label="创建时间" prop="createdAt">
          <el-date-picker clearable
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        category: [
          { required: true, message: "流程分类(如：请假、报销等)不能为空", trigger: "blur" }
        ],
        creatorId: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除审批流程定义编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/processes/export', {
        ...this.queryParams
